    ...
    while True:
        ...
        if(choice_1 in ["1","9","0"]):
            if (choice_1 == "1"):
                ...
                p = nextprime(secrets.randbits(2048))
                q = nextprime(secrets.randbits(2048))
                e = 65537
                n = q * p
                Fn = (q-1) * (p-1)
                d = mod_inverse (e, Fn)
...
(4) Язык (ru-en) - {data_settings.get("language")} --- ДОСТУПНО ТОЛЬКО RU
(5) Цветной режим - {data_settings.get("colorama")} --- ДОСТУПНО ТОЛЬКО ВКЛ
(9) Выйти в главное меню
(0) Выйти
''')
